...
class CjCallableReferenceExpression(node: ASTNode) : CjExpressionImpl(node), CjDoubleColonExpression {
    val callableReference: CjSimpleNameExpression
        get() {
            var psi: PsiElement? = doubleColonTokenReference
            while (psi != null) {
                if (psi is CjSimpleNameExpression) {
                    return psi
                }
                psi = psi.nextSibling
            }

            throw IllegalStateException("Callable reference simple name shouldn't be parsed to null")
        }

    override fun findColonColon(): PsiElement? {
        return null
