...
import org.cangnova.cangjie.toolchain.toPath
...
class CjpmBuildSystem : CangJieBuildSystem {
    ...
    override fun isApplicable(project: Project, projectTask: ProjectTask): Boolean {
        return projectTask is ModuleBuildTask && projectTask.module.getOptionValue(ELEMENT_TYPE) == "CANGJIE_MODULE" && BuildSettingsState.getInstance(
            project
        ).defaultBuildSystemId == id && CangJieToolchain.getToolchain().isAvailable && project.basePath.toPath().resolve("")
    ...
    override fun createBuilder(project: Project): CangJieProjectBuilder {
        ...
        return CjpmProjectBuilder(project)
    }
}

internal class CjpmProjectBuilder(val project: Project) : CangJieProjectBuilder {
