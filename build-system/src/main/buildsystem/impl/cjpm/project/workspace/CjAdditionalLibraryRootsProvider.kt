...
private val CjpmProject.ideaLibraries: Collection<SyntheticLibrary>
    get() {
        ...
        for (pkg in workspace.packages) {
            when (pkg.origin) {
                PackageOrigin.STDLIB, PackageOrigin.STDLIB_DEPENDENCY -> stdlibPackages += pkg
                PackageOrigin.DEPENDENCY -> dependencyPackages += pkg
                PackageOrigin.WORKSPACE -> Unit
            }
        }
...
private fun makeStdlibLibrary(packages: List<CjpmWorkspace.Package>, cjcVersion: SdkVersion?): CjpmLibrary? {
