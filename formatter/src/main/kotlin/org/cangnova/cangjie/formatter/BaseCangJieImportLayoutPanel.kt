    ...
    protected fun resizeColumns() {
        val packageRenderer: ColoredTableCellRenderer = object : ColoredTableCellRenderer() {
            override fun customizeCellRenderer(
                ...
                append(CangJieFormatterBundle.message("import.text.import"), SimpleTextAttributes.fromTextAttributes(attributes))
                append(" ", SimpleTextAttributes.REGULAR_ATTRIBUTES)

                when (entry) {
                    CangJiePackageEntry.ALL_OTHER_IMPORTS_ENTRY -> append(
                        CangJieFormatterBundle.message("import.text.all.other.imports"),
                        SimpleTextAttributes.REGULAR_ATTRIBUTES
                    )

                    CangJiePackageEntry.ALL_OTHER_ALIAS_IMPORTS_ENTRY -> append(
                        CangJieFormatterBundle.message("import.text.all.alias.imports"),
                        SimpleTextAttributes.REGULAR_ATTRIBUTES
                    )
...
class CangJieStarImportLayoutPanel : BaseCangJieImportLayoutPanel(CangJieFormatterBundle.message("title.packages.to.use.import.with")) {
    init {
        val importLayoutPanel = ToolbarDecorator.createDecorator(layoutTable)
            .setAddAction { addPackage() }
            .setRemoveAction { removePackage() }
            .setButtonComparator(
                CangJieFormatterBundle.message("start.import.button.text.add"),
                CangJieFormatterBundle.message("start.import.button.text.remove")
...
class CangJieImportOrderLayoutPanel : BaseCangJieImportLayoutPanel(CangJieFormatterBundle.message("title.import.layout")) {
    private val cbImportAliasesSeparately = JBCheckBox(CangJieFormatterBundle.message("codestyle.layout.import.aliases.separately"))
    ...
    init {
        ...
        val importLayoutPanel = ToolbarDecorator.createDecorator(layoutTable)
            .addExtraAction(
                object: AnAction(CangJieFormatterBundle.message("button.add.package"),null, IconUtil.addPackageIcon){
            ...
            .setRemoveActionUpdater {
                ...
                CangJieFormatterBundle.message("import.order.button.text.add.package"),
                CangJieFormatterBundle.message("import.order.button.text.remove"),
                CangJieFormatterBundle.message("import.order.button.text.up"),
                CangJieFormatterBundle.message("import.order.button.text.down")
            ).setPreferredSize(Dimension(-1, 100))
            .createPanel()
...
fun createTableForPackageEntries(packageTable: CangJiePackageEntryTable): JBTable {
    val names = arrayOf(CangJieFormatterBundle.message("listbox.import.package"), CangJieFormatterBundle.message("listbox.import.with.subpackages"))
