...
class CangJieOtherSettingsPanel(settings: CodeStyleSettings) : CodeStyleAbstractPanel(CangJieLanguage, null, settings) {
    private val cbTrailingComma = JCheckBox(CangJieFormatterBundle.message("formatter.checkbox.text.use.trailing.comma"))
    ...
    override fun getTabTitle(): String = CangJieFormatterBundle.message("formatter.title.other")
...
//                    add(
//                        OptionGroup(CangJieFormatterBundle.message("formatter.title.trailing.comma")).apply {
//                            add(cbTrailingComma)
//                        }.createPanel()
//                    )
//                }
        ...
        add(
            JBScrollPane(
                JPanel().apply {
                    layout = BoxLayout(this, BoxLayout.Y_AXIS)
                    border = BorderFactory.createEmptyBorder(0, UIUtil.DEFAULT_HGAP, UIUtil.DEFAULT_VGAP, UIUtil.DEFAULT_HGAP)
                    add(JLabel(CangJieFormatterBundle.message("formatter.title.trailing.comma")))
                    add(cbTrailingComma)
                }
            )
        )
