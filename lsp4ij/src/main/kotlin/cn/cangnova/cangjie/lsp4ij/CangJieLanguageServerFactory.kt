...
class CangJieLanguageServerFactory : LanguageServerFactory {
    private lateinit var connectionProvider: CangJieStreamConnectionProvider
    override fun createConnectionProvider(p0: Project): StreamConnectionProvider {

        if (!this::connectionProvider.isInitialized) {
            connectionProvider = CangJieStreamConnectionProvider(p0)
        }
    ...
    override fun createClientFeatures(): LSPClientFeatures = object : LSPClientFeatures() {
        ...
        fun lowercaseWindowsDriveAndEscapeColon(uri: String): String {
          ...
          fun VirtualFile.getFileUri( ): String? {
            val escapedPath = URLUtil.encodePath(getFilePath(this))
        ...
        override fun getFileUri(file: VirtualFile): URI? {
            ...
            val url = VirtualFileManager.constructUrl(URLUtil.FILE_PROTOCOL, escapedPath)
            val uri = VfsUtil.toUri(url)?.toString() ?: url
            return URI.create(lowercaseWindowsDriveAndEscapeColon(uri))
        }
...
//                getFileUri(project.guessProjectDir()!!)?.path
                ...
                fun getMap(): Map<String, Any> {
                    return mapOf(
                        ...
                        "multiModuleOption" to mutableMapOf<String, Any>(
                            project.guessProjectDir()!!.      getFileUri()!! to mapOf(
                                "name" to projectName,
...
class CangJieStreamConnectionProvider(project: Project) :
    OSProcessStreamConnectionProvider(CangJieLspServerManager.getCommandLine(project)) {
    ...
    private fun interceptAndModifyOutput() {
        try {
            val reader = BufferedReader(InputStreamReader(super.inputStream))
    ...
    override fun start() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            interceptAndModifyOutput()
        }
    }

    override fun getInputStream(): InputStream? = modifiedInputStream
