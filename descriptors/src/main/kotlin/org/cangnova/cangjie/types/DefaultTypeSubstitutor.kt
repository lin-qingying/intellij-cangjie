 ...
 *     T.typeConstructor -> TypeArgument(Variance.INVARIANT, IntType)
 * ))
 *
 * // 应用替换: List<T> -> List<Int>
 * val substitutedType = substitutor.substitute(listTType)
 * ```
 *
 * ### 递归替换
 *
 ...
 *
 * `DefaultTypeSubstitutor` 和 `LazySubstitutingClassDescriptor` 协同工作实现泛型：
 *
 * ```
 * 泛型类 A<T>
 *     ↓
 ...
 * // 显式指定映射
 * val substitutor = DefaultTypeSubstitutor.create(mapOf(
 *     TConstructor to TypeArgumentImpl(Variance.INVARIANT, IntType)
 * ))
 * ```
 *
 * ### 链式替换
 * ```kotlin
 * // 组合多个替换器
 * val first = DefaultTypeSubstitutor.create(mapOf(T -> A<U>))
 * val second = DefaultTypeSubstitutor.create(mapOf(U -> Int))
 * val chained = DefaultTypeSubstitutor.createChainedSubstitutor(
 *     first.substitution,
 *     second.substitution
 * )
 * // 最终效果: T -> A<Int>
 ...
 * ```kotlin
 * // 替换类型投影（包含型变信息）
 * fun substitute(TypeArgument: TypeArgument): TypeArgument?
 *
 * // 替换类型（指定使用位置的型变）
 ...
 * ```kotlin
 * // 用于内部处理，不对捕获类型进行近似
 * fun substituteWithoutApproximation(TypeArgument: TypeArgument): TypeArgument?
 * ```
 *
 * ## 性能优化
 *
 ...
 * // 创建替换器: T -> String
 * val substitutor = DefaultTypeSubstitutor.create(mapOf(
 *     tParam.typeConstructor to TypeArgumentImpl(Variance.INVARIANT, stringType)
 * ))
 *
 * // 应用替换
 ...
 *
 * // 创建替换器（自动提取 E -> Int）
 * val substitutor = DefaultTypeSubstitutor.create(listIntType)
 *
 ...
 * // 创建 Container<String>
 * val substitutor = DefaultTypeSubstitutor.create(mapOf(
 *     TConstructor to TypeArgumentImpl(Variance.INVARIANT, stringType)
 * ))
 *
 * // 获取 items 属性的类型
 ...
 *
 * // 应用替换
 * val substitutedType = substitutor.substitute(itemsType)
 * // 结果: List<String>
 * ```
 *
 * ## 错误处理
...
class DefaultTypeSubstitutor(val substitution: TypeSubstitution) : TypeSubstitutor {
    ...
    override fun safeSubstitute(type: UnwrappedType): UnwrappedType {
        return safeSubstitute(type as CangJieType).unwrap()
    }

    override val isEmpty: Boolean
        get() = substitution.isEmpty()

    // DefaultTypeSubstitutor 原有方法
    fun substituteWithoutApproximation(TypeArgument: TypeArgument): TypeArgument? {
        if (isEmpty) {
            return TypeArgument
        }
        ...
        return try {
            unsafeSubstitute(TypeArgument, null, 0)
    ...
    fun substitute(TypeArgument: TypeArgument): TypeArgument? {
        val substitutedTypeArgument = substituteWithoutApproximation(TypeArgument)
        if (!substitution.approximateCapturedTypes()) {
            return substitutedTypeArgument
        }
        // 仓颉语言不需要 approximateContravariantCapturedTypes 参数
        return approximateCapturedTypesIfNecessary(substitutedTypeArgument)
    }

    fun substitute(type: CangJieType ): CangJieType? {
        val projection =
            substitute(TypeArgumentImpl(  substitution.prepareTopLevelType(type )))
    ...
    private fun unsafeSubstitute(
        ...
        if (type is TypeWithEnhancement) {
            ...
            val substitution: TypeArgument = unsafeSubstitute(
                TypeArgumentImpl(  origin),
                typeParameter,
                recursionDepth + 1
            )
        ...
        val substituted: TypeArgument? = substitution[type]
        val replacement: TypeArgument? =
        ...
        if (replacement == null && type.isFlexible() && !type.isCustomTypeParameter()) {
            ...
            val substitutedLower: TypeArgument =
                unsafeSubstitute(
                    TypeArgumentImpl(  flexibleType.lowerBound),
                    typeParameter,
                    recursionDepth + 1
                )
            val substitutedUpper: TypeArgument =
                unsafeSubstitute(
                    TypeArgumentImpl(  flexibleType.upperBound),
                    typeParameter,
                    recursionDepth + 1
                )
            ...
            return TypeArgumentImpl(  substitutedFlexibleType)
        }
...
//                    DefaultTypeSubstitutor.filterOutUnsafeVariance(
...
//            }
    ...
    private fun substituteCompoundType(
        ...
        val substitutedAbbreviation = type.getAbbreviation()?.let { abbreviation ->
            ...
            substitutorForAbbreviation.substitute(abbreviation )
        }
    ...
    private fun substituteTypeArguments(
        ...
        val substitutedArguments = ArrayList<TypeArgument>(typeParameters.size)
    ...
    fun safeSubstitute(type: CangJieType ): CangJieType {
        if (isEmpty) {
            return type
        }

        return try {
            unsafeSubstitute(TypeArgumentImpl(  type), null, 0).type
        } catch (e: SubstitutionException) {
            createErrorType(
    ...
    companion object {
        @JvmField
        val EMPTY: DefaultTypeSubstitutor = create(TypeSubstitution.EMPTY)

        @JvmStatic

        fun createChainedSubstitutor(first: TypeSubstitution, second: TypeSubstitution): DefaultTypeSubstitutor {
            return create(create(first, second))
        }

        @JvmStatic

        fun create(substitutionContext: Map<TypeConstructor, TypeArgument>): DefaultTypeSubstitutor {
        ...
        fun create(context: CangJieType): DefaultTypeSubstitutor {
        ...
        @JvmStatic
        fun create(substitution: TypeSubstitution): DefaultTypeSubstitutor {
            return DefaultTypeSubstitutor(substitution)
        }
    }
