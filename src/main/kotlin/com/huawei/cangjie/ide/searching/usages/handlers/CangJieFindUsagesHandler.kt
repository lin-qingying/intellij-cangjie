...
import com.huawei.cangjie.ide.runReadActionInSmartMode
import com.huawei.cangjie.ide.searching.usages.CangJieFindUsagesHandlerFactory
import com.huawei.cangjie.ide.searching.usages.CangJieReferencePreservingUsageInfo
import com.huawei.cangjie.ide.searching.usages.CangJieReferenceUsageInfo
...
abstract class CangJieFindUsagesHandler<T : PsiElement>(
    ...
    constructor(psiElement: T, factory: CangJieFindUsagesHandlerFactory) : this(psiElement, emptyList(), factory)
    ...
    companion object {
        val LOG = Logger.getInstance(CangJieFindUsagesHandler::class.java)

        internal fun processUsage(processor: Processor<in UsageInfo>, ref: PsiReference): Boolean =
            processor.processIfNotNull {
                when {
//                    ref is LightMemberReference -> CangJieReferencePreservingUsageInfo(ref)
                    ref.element.isValid -> CangJieReferenceUsageInfo(ref)
                    else -> null
                }
            }
