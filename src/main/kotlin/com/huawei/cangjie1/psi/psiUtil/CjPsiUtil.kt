...
fun StubBasedPsiElementBase<out CangJieClassOrObjectStub<out CjClassOrObject>>.getSuperNames(): List<String> {
    ...
    require(this is CjClassOrObject) { "it should be ${CjClassOrObject::class} but it is a ${this::class.java.name}" }
...
val CjDeclaration.containingClassOrObject: CjClassOrObject?
    get() = parent.let {
        when (it) {
            is CjClassBody -> it.parent as? CjClassOrObject
            is CjClassOrObject -> it
            is CjParameterList -> (it.parent as? CjPrimaryConstructor)?.getContainingClassOrObject()
            else -> null
        }
    }
