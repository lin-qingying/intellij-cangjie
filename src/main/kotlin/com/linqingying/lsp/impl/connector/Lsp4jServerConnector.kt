...
abstract class Lsp4jServerConnector(lspServer: LspServer) {
    ...
    fun connect() {
        ...
        val remoteEndpoint = RemoteEndpoint(StreamMessageConsumer(getServerOutputStream(), messageHandler), ServiceEndpoints.toEndpoint(lsp4jClient))
        messageHandler.methodProvider = remoteEndpoint
        lsp4jServer = ServiceEndpoints.toServiceObject(remoteEndpoint, serverDescriptor.lsp4jServerClass) as LanguageServer
        ApplicationManager.getApplication().executeOnPooledThread {
            ConcurrencyUtil.runUnderThreadName("LSP Listener: $serverDescriptor") {
                ...
                try {
                    val messageProducer = StreamMessageProducer(getServerInputStream(), messageHandler)
                    try {
