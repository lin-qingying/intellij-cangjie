...
public final class CangJieClassInheritorsSearcher extends QueryExecutorBase<CjTypeStatement, ClassInheritorsSearch.SearchParameters> {
    @Override
    public void processQuery(@NotNull ClassInheritorsSearch.SearchParameters parameters, @NotNull Processor<? super CjTypeStatement> consumer) {
        final CjTypeStatement baseClass = parameters.getClassToProcess();
    ...
    private static void processInheritors(@NotNull final ClassInheritorsSearch.SearchParameters parameters,
                                          @NotNull final Processor<? super CjTypeStatement> consumer) {
        @NotNull final CjTypeStatement baseClass = parameters.getClassToProcess();
...
//        }
        ...
        Iterable<CjTypeStatement> cached = getOrComputeSubClasses(project, baseClass, searchScope, parameters);

        for (final CjTypeStatement subClass : cached) {
            ProgressManager.checkCanceled();
            if (subClass == null) {
    ...
    @NotNull
    private static Iterable<@NotNull CjTypeStatement> getOrComputeSubClasses(@NotNull Project project,
                                                                      @NotNull CjTypeStatement baseClass,
        ...
        ConcurrentMap<CjTypeStatement, Iterable<CjTypeStatement>> map = parameters.isIncludeAnonymous()
                ? caches.ALL_SUB_CLASSES
                : caches.ALL_SUB_CLASSES_NO_ANONYMOUS;
        Iterable<CjTypeStatement> cached = map.get(baseClass);
        if (cached == null) {
            // returns lazy collection of subclasses. Each call to next() leads to calculation of next batch of subclasses.
            Function<@NotNull PsiAnchor, @NotNull CjTypeStatement> converter =
                    anchor -> ReadAction.compute(() -> (@NotNull CjTypeStatement)anchor.retrieve());
            Predicate<CjTypeStatement> applicableFilter =
                    Objects::nonNull;
            ...
            LazyConcurrentCollection.MoreElementsGenerator<PsiAnchor, CjTypeStatement> generator = (candidate, processor) -> DirectClassInheritorsSearch
            ...
            Iterable<CjTypeStatement> computed = new LazyConcurrentCollection<>(seed, converter, applicableFilter, generator);
    ...
    private static void processLocalScope(@NotNull final Project project,
                                          ...
                                          @NotNull CjTypeStatement baseClass,
                                          @NotNull Processor<? super CjTypeStatement> consumer) {
    ...
    private static void processFile(PsiFile psiFile,
                                    final Processor<? super CjTypeStatement> consumer,
                                    final ClassInheritorsSearch.@NotNull SearchParameters parameters,
                                    @NotNull final CjTypeStatement baseClass, final boolean[] success) {
...
//        });
    ...
    private static boolean checkCandidate(@NotNull CjTypeStatement candidate, @NotNull ClassInheritorsSearch.SearchParameters parameters) {
