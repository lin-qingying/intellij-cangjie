...
import com.linqingying.cangjie.builtins.functions.isBasicFunction
...
import com.linqingying.cangjie.resolve.calls.model.LambdaWithTypeVariableAsExpectedTypeMarker
import com.linqingying.cangjie.resolve.calls.model.PostponedAtomWithRevisableExpectedType
import com.linqingying.cangjie.resolve.calls.model.PostponedCallableReferenceMarker
...
//    private val languageVersionSettings: LanguageVersionSettings,
    ...
    private fun Context.computeParameterInfoFromRelatedLambdas(
        ...
        val parameterTypesFromDeclarationOfRelatedLambdas: List<Pair<List<CangJieTypeMarker?>, Boolean>> = postponedArguments
    ...
    fun collectParameterTypesAndBuildNewExpectedTypes(
        ...
        return postponedArguments.any { argument ->
            ...
            val newExpectedType =
                c.buildNewFunctionalExpectedType(
                    argument,
                    parameterTypesInfo,
                    dependencyProvider,
    ...
    private fun Context.computeTypeVariablePathInsideGivenType(
        ...
        targetVariable: TypeConstructorMarker,
        path: java.util.Stack<Pair<TypeConstructorMarker, Int>> = java.util.Stack()
    ...
    private fun Context.buildNewFunctionalExpectedType(
        ...
        val pathFromRelatedTopLevelVariable = if (relatedTopLevelVariable != null) {
            val constraintTypes =
                notFixedTypeVariables.getValue(relatedTopLevelVariable).constraints.map { it.type }.toSet()
    ...
    private fun getDeclaredParametersConsideringExtensionFunctionsPresence(parameterTypesInfo: ParameterTypesInfo): List<CangJieTypeMarker?>? =
    ...
    private fun PostponedResolvedAtomMarker.expectedFunctionType(c: Context): CangJieTypeMarker? = with(c) {
        val expectedType =
            (this@expectedFunctionType as? PostponedAtomWithRevisableExpectedType)?.revisedExpectedType ?: expectedType
        ...
        when {
            ...
            type.argumentsCount() > 0 -> {
                for (typeArgument in type.lowerBoundIfFlexible().asArgumentList()) {
                    ...
                    getAllDeeplyRelatedTypeVariables(
    ...
    private fun Context.fixNextReadyVariableForParameterType(
        ...
        val variableForFixation =
            findNextVariableForParameterType(type, dependencyProvider, postponedArguments, topLevelType)
