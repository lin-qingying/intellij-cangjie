...
import org.cangnova.cangjie.ide.run.cjpm.runconfig.CjCapturingProcessHandler
import org.cangnova.cangjie.ide.run.cjpm.runconfig.CjProcessExecutionException
import org.cangnova.cangjie.ide.run.cjpm.runconfig.unwrapOrElse
import org.cangnova.cangjie.toolchain.impl.CangJieVersion.Companion.getInfo
...
class Cjc(toolchain: org.cangnova.cangjie.toolchain.CjToolchainBase) : CangJieComponent(NAME, toolchain) {

    val version: org.cangnova.cangjie.toolchain.impl.CangJieVersion?
        get() = getInfo()
    ...
    fun queryVersion(workingDirectory: Path? = null): org.cangnova.cangjie.toolchain.impl.CangJieVersion? {
        try {
            if (!isUnitTestMode) {
            ...
            return lines?.let { org.cangnova.cangjie.toolchain.impl.parseCjcVersion(it) }
    ...
    ): CjProcessResult<org.cangnova.cangjie.toolchain.impl.CangJieVersion?> {
        if (!isUnitTestMode) {
            checkIsBackgroundThread()
        }
        return createBaseCommandLine("-v", workingDirectory = workingDirectory).execute(owner, listener = listener)
            .map {
                org.cangnova.cangjie.toolchain.impl.parseCjcVersion(it.stdoutLines)
