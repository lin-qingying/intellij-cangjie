...
DIGITS={DIGIT}({DIGIT_OR_UNDERSCORE}*{DIGIT})?  // 数字序列，允许中间有下划线
...
// 各种进制的整数表示
DECIMAL_INTEGER_LITERAL=0|[1-9]({DIGIT_OR_UNDERSCORE}*{DIGIT})?  // 十进制整数
HEX_INTEGER_LITERAL=0[Xx]({HEX_DIGIT}({HEX_DIGIT_OR_UNDERSCORE}*{HEX_DIGIT})?)  // 十六进制整数
BIN_INTEGER_LITERAL=0[Bb]({BIN_DIGIT}({BIN_DIGIT_OR_UNDERSCORE}*{BIN_DIGIT})?)  // 二进制整数
OCT_INTEGER_LITERAL=0[Oo]({OCT_DIGIT}({OCT_DIGIT_OR_UNDERSCORE}*{OCT_DIGIT})?)  // 八进制整数
...
FLOATING_POINT_LITERAL1=({DIGITS})"."({DIGITS})?({EXPONENT_PART})?  // 123.45, 123.45e10
FLOATING_POINT_LITERAL2="."({DIGITS})({EXPONENT_PART})?             // .45, .45e10
FLOATING_POINT_LITERAL3=({DIGITS})({EXPONENT_PART})                 // 123e10

// 十六进制浮点数
HEX_FLOATING_POINT_LITERAL={HEX_FLOATING_POINT_LITERAL1}|{HEX_FLOATING_POINT_LITERAL2}|{HEX_FLOATING_POINT_LITERAL3}
HEX_FLOATING_POINT_LITERAL1=0[xX]({HEX_DIGIT}({HEX_DIGIT_OR_UNDERSCORE}*{HEX_DIGIT})?)"."({HEX_DIGIT}({HEX_DIGIT_OR_UNDERSCORE}*{HEX_DIGIT})?)"p"[+-]?{DIGITS}  // 0x1.8p1
HEX_FLOATING_POINT_LITERAL2=0[xX]"."({HEX_DIGIT}({HEX_DIGIT_OR_UNDERSCORE}*{HEX_DIGIT})?)"p"[+-]?{DIGITS}  // 0x.8p1
HEX_FLOATING_POINT_LITERAL3=0[xX]({HEX_DIGIT}({HEX_DIGIT_OR_UNDERSCORE}*{HEX_DIGIT})?)"p"[+-]?{DIGITS}  // 0x1p1

// 指数部分
EXPONENT_PART=[Ee][+-]?{DIGITS}      // 科学计数法的指数部分，如e10, e+10, e-10
