...
import cn.cangnova.cangjie.ide.codeinsight.hints.InlayInfoDetail
import cn.cangnova.cangjie.ide.codeinsight.hints.TextInlayInfoDetail
import cn.cangnova.cangjie.ide.codeinsight.hints.TypeInlayInfoDetail
import cn.cangnova.cangjie.ide.codeinsight.quickDoc.cdoc.CangJieIdeDescriptorRenderer
import cn.cangnova.cangjie.ide.codeinsight.quickDoc.cdoc.ClassifierNamePolicyEx
...
import cn.cangnova.cangjie.renderer.*
...
import cn.cangnova.cangjie.resolve.descriptorUtil.parentsWithSelf
 ...
 */
class HintsTypeRenderer private constructor(override val options: HintsDescriptorRendererOptions) :
    CangJieIdeDescriptorRenderer(options) {
    ...
    /* TYPES RENDERING */
    fun renderTypeIntoInlayInfo(type: CangJieType): List<InlayInfoDetail> {
    ...
    private fun MutableList<InlayInfoDetail>.append(
        text: String,
        descriptor: ClassifierDescriptor?
    ): MutableList<InlayInfoDetail> {
        descriptor?.let {
    ...
    private fun CangJieType.renderNormalizedTypeTo(list: MutableList<InlayInfoDetail>): MutableList<InlayInfoDetail> {
    ...
    private fun CangJieType.renderNormalizedTypeAsIsTo(list: MutableList<InlayInfoDetail>) {
    ...
    private fun CangJieType.renderDefaultTypeTo(list: MutableList<InlayInfoDetail>) {
//        renderAnnotationsTo(list)
    ...
    private fun CangJieType.renderTypeConstructorAndArgumentsTo(
    ...
    private fun TypeConstructor.renderTypeConstructorOfTypeTo(list: MutableList<InlayInfoDetail>, type: CangJieType) {
        val text = when (val cd = this.declarationDescriptor) {
            is TypeParameterDescriptor, is ClassDescriptor, is TypeAliasDescriptor -> renderClassifierNameWithType(
                cd,
                type
            )
...
//        }
        ...
        val isNullable = this.isMarkedOption
        val receiverType = this.getReceiverTypeFromFunctionType()

        val needParenthesis = isNullable || (receiverType != null)
        if (needParenthesis) {
    ...
    private fun Annotated.renderAnnotationsTo(
        list: MutableList<InlayInfoDetail>,
        ...
        val excluded =
            if (this is CangJieType) options.excludedTypeAnnotationClasses else options.excludedAnnotationClasses
...
//    }
    ...
    companion object {
        ...
        internal fun getInlayHintsTypeRenderer(bindingContext: BindingContext, context: CjElement) =
            withOptions {
                modifiers = emptySet()
    ...
    internal class ImportAwareClassifierNamePolicy(
        ...
        private fun shortNameWithCompanionNameSkip(
