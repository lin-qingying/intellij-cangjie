...
import cn.cangnova.cangjie.psi.psiUtil.getCallNameExpression
...
class CangJieMultilineStringEnterHandler : EnterHandlerDelegate {
    ...
    override fun preprocessEnter(
        ...
        val hostPosition = getHostPosition(dataContext) ?: return EnterHandlerDelegate.Result.Continue
    ...
    private fun preprocessEnter(
        ...
        if (file.fileType != CangJieFileType.INSTANCE) return EnterHandlerDelegate.Result.Continue
    ...
    private fun postProcessEnter(file: PsiFile, editor: Editor): EnterHandlerDelegate.Result {
        if (file.fileType != CangJieFileType.INSTANCE) return EnterHandlerDelegate.Result.Continue

        if (!wasInMultilineString) return EnterHandlerDelegate.Result.Continue
        ...
        val literal = findString(element, offset) ?: return EnterHandlerDelegate.Result.Continue
        ...
        runWriteAction {
            ...
            val nextLine =
                if (document.lineCount > prevLineNumber + 2) getLineByNumber(prevLineNumber + 2, document) else ""

            val wasSingleLine =
                literal.text.indexOf("\n") == literal.text.lastIndexOf("\n") // Only one '\n' in the string after insertion
            val lines = literal.text.split("\n")

            val literalOffset = literal.textRange.startOffset
            if (wasSingleLine || (lines.size == 3 && isInBrace)) {
                val shouldUseTrimIndent =
                    hasTrimIndentCallInChain || (marginChar == null && lines.first().trim() == MULTILINE_QUOTE)
                ...
                val nonBlankNotFirstLines =
                    lines.subList(1, lines.size).filterNot { it.isBlank() || it.trimStart() == MULTILINE_QUOTE }
    ...
    companion object {
        ...
        fun findString(element: PsiElement?, offset: Int): CjStringTemplateExpression? {
            ...
            when (element.elementType) {
                ...
                CjTokens.CLOSING_QUOTE, CjTokens.SHORT_TEMPLATE_ENTRY_START, CjTokens.LONG_TEMPLATE_ENTRY_START -> {
        ...
        fun getMarginCharFromLiteral(
            str: CjStringTemplateExpression,
            marginChar: Char = DEFAULT_TRIM_MARGIN_CHAR
        ): Char? {
        ...
        private fun getLiteralCalls(str: CjStringTemplateExpression): Sequence<CjCallExpression> {
            var previous: PsiElement = str
            return str.parents
                .takeWhile { parent ->
                    if (parent is CjQualifiedExpression && parent.receiverExpression == previous) {
                        previous = parent
                        true
                    } else {
                        false
                ...
                .mapNotNull { qualified ->
                    (qualified as CjQualifiedExpression).selectorExpression as? CjCallExpression
                }
        }

        fun getMarginCharFromTrimMarginCallsInChain(str: CjStringTemplateExpression): Char? {
            ...
            val argumentExpression =
                firstArgument.getArgumentExpression() as? CjStringTemplateExpression ?: return DEFAULT_TRIM_MARGIN_CHAR
            val entry = argumentExpression.entries.singleOrNull() as? CjLiteralStringTemplateEntry
                ?: return DEFAULT_TRIM_MARGIN_CHAR

            return entry.text?.singleOrNull() ?: DEFAULT_TRIM_MARGIN_CHAR
        }

        fun hasTrimIndentCallInChain(str: CjStringTemplateExpression): Boolean {
