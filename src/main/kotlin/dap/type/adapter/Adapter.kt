...
class BreakpointEventReasonAdapter {
    @ToJson
    fun toJson(writer: JsonWriter, breakpointEventReason: BreakpointEventReason) {
        when (breakpointEventReason) {
            is BreakpointEventReason.changed -> writer.value("changed")
            is BreakpointEventReason.new -> writer.value("new")
            is BreakpointEventReason.removed -> writer.value("removed")
            is BreakpointEventReason.Other -> writer.value(breakpointEventReason.value)
        }
    }

    @FromJson
    fun fromJson(reader: JsonReader): BreakpointEventReason {
        return when (val value = reader.nextString()) {
