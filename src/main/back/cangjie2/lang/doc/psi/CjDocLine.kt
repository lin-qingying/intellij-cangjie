...
data class CjDocLine(
    ...
    fun removePrefix(prefix: String): CjDocLine {
    ...
    fun removeSuffix(suffix: String): CjDocLine {
    ...
    fun substring(startIndex: Int): CjDocLine {
    ...
    companion object {
        fun splitLines(text: CharSequence): Sequence<CjDocLine> {
            var prev = 0
            return generateSequence {
                if (prev == -1) return@generateSequence null
                val index = text.indexOf(char = '\n', startIndex = prev)
                if (index >= 0) {
                    val line = CjDocLine(text, startOffset = prev, endOffset = index, isLastLine = false)
                    prev = index + 1
                    line
                } else {
                    val line = CjDocLine(text, startOffset = prev, endOffset = text.length, isLastLine = true)
                    prev = -1
                    line
                }
            }
