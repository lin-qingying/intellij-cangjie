...
import cn.cangnova.cangjie.serialization.deserialization.DeserializationContext
import cn.cangnova.cangjie.serialization.deserialization.MemberDeserializer
...
abstract class DeserializedMemberScope protected constructor(
    ...
    variableList: List<ProtoBuf.Variable>,
     ...
     * [cn.cangnova.cangjie.serialization.deserialization.DeserializationConfiguration.preserveDeclarationsOrdering] is
    ...
    private val impl: Implementation = createImplementation(functionList, variableList, propertyList, typeAliasList)
    ...
    private inner class OptimizedImplementation(
        ...
        private val variableProtosBytes = variableList.groupByName { it.name }.packToByteArray()
        ...
        private fun Map<Name, Collection<AbstractMessage>>.packToByteArray(): Map<Name, ByteArray> =
            mapValues { entry ->
        ...
        override val propertyNames by c.storageManager.createLazyValue {
            propertyProtosBytes.keys + getNonDeclaredPropertyNames()
        }
        override val variableNames by c.storageManager.createLazyValue {
            variableProtosBytes.keys + getNonDeclaredVariableNames()
        }
        ...
        private fun computeVariables(name: Name) =
            computeDescriptors(
                name,
                propertyProtosBytes,
                ProtoBuf.Variable.parser(),
                { c.memberDeserializer.loadVariable(it) },
    ...
    private inner class NoReorderImplementation(
        ...
        private val variableList: List<ProtoBuf.Variable>,
    ...
    protected abstract fun addEnumEntryDescriptors(
        result: MutableCollection<DeclarationDescriptor>,
        nameFilter: (Name) -> Boolean
    )
    ...
    override fun getPropertyNames() = impl.propertyNames
