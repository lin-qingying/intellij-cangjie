...
import cn.cangnova.cangjie.builtins.contextFunctionTypeParamsCount
...
import cn.cangnova.cangjie.resolve.calls.inference.constraintPosition.ConstraintPosition
import cn.cangnova.cangjie.resolve.calls.inference.constraintPosition.ConstraintPositionKind
import cn.cangnova.cangjie.resolve.calls.inference.model.TypeVariable
...
import cn.cangnova.cangjie.types.checker.TypeCheckingProcedureCallbacks
...
import cn.cangnova.cangjie.types.model.requireOrDescribe
...
import cn.cangnova.cangjie.types.util.defaultProjections
import cn.cangnova.cangjie.types.util.isDefaultBound
import java.util.*
...
//        if (subType.hasNoInferAnnotation() || superType.hasNoInferAnnotation()) return
    ...
    private fun generateTypeParameterCaptureConstraint(
        ...
        val typeProjection = if (isTypeMarkedNullable) {
            TypeProjectionImpl(
                constrainingTypeProjection.projectionKind,
                TypeUtils.makeNotNullable(constrainingTypeProjection.type)
            )
        } else {
    ...
    fun addConstraint(
        ...
        val typeCheckingProcedure = TypeCheckingProcedure(object : TypeCheckingProcedureCallbacks {
            ...
            override fun assertEqualTypes(
                ...
                depth++
                doAddConstraint(ConstraintKind.EQUAL, a, b, newConstraintContext, typeCheckingProcedure)
                depth--
                return true
