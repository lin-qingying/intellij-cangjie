package cn.cangnova.cangjie.lsp4intellij.lsp4intellij
...
import cn.cangnova.cangjie.lsp4intellij.client.connection.ProcessStreamConnectionProvider
import cn.cangnova.cangjie.lsp4intellij.client.connection.StreamConnectionProvider
import cn.cangnova.cangjie.lsp4intellij.client.languageserver.serverdefinition.ProcessBuilderServerDefinition
import cn.cangnova.cangjie.lsp4intellij.client.languageserver.serverdefinition.ServerListener
import java.io.*
...
private class MyServerDefinition(val project: Project, command: GeneralCommandLine) :
    ProcessBuilderServerDefinition("cj,toml", command.toProcessBuilder()) {
    init {
        languageIds = mapOf(
            "cj" to "Cangjie", "toml" to "Toml"
        )
    ...
    override fun createConnectionProvider(workingDir: String?): StreamConnectionProvider? {
        return InterceptingProcessStreamConnectionProvider(processBuilder)
    }

    override fun getServerListener(): ServerListener? {
        return super.getServerListener()
    }
    ...
    override fun customizeInitializeParams(initializeParams: InitializeParams) {

        val toolchain = project.cangjieSettings.toolchain


        initializeParams.rootPath = project.guessProjectDir()?.path
        initializeParams.rootUri = getFileUri(project.guessProjectDir()!!)
...
class InterceptingProcessStreamConnectionProvider(
    processBuilder: ProcessBuilder
) : ProcessStreamConnectionProvider(processBuilder) {
    ...
    fun readNewlineChars(reader: PushbackReader): String {  // 直接使用 PushbackReader 类型
...
fun getDefaultGsonBuilder(): GsonBuilder {
    return GsonBuilder()
        ...
        .registerTypeAdapterFactory(TupleTypeAdapters.TwoTypeAdapterFactory())
