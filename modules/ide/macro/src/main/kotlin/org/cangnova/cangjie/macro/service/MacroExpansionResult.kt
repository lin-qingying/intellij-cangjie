...
data class MacroExpansionResult(
     ...
     * 展开结果的来源
     *
     * 使用 [MacroExpansionSource] 而非简单枚举，以支持携带引擎元信息。
     */
    val source: MacroExpansionSource
)
 ...
 * 宏展开结果来源
 *
 * 使用 sealed class 而非枚举，允许引擎自身携带元信息（[MacroExpansionEngine]），
 * 遵循"由来源者自行实现相关信息"的设计原则。
 */
sealed class MacroExpansionSource {

    /**
     * 结果来自缓存
     *
     * 原始展开结果由某个引擎产生并被缓存，本次直接从缓存返回。
     *
     * @param originalEngine 产生原始结果的引擎，若缓存条目未记录则为 null
     */
    data class Cache(val originalEngine: MacroExpansionEngine? = null) : MacroExpansionSource() {
        override fun toString(): String = "Cache(${originalEngine?.displayName ?: "unknown"})"
    }

    /**
     * 结果来自特定引擎的实时展开
     *
     * @param engine 执行展开的引擎
     */
    data class Engine(val engine: MacroExpansionEngine) : MacroExpansionSource() {
        override fun toString(): String = "Engine(${engine.displayName})"
    }
}

/**
 * 展开结果来源（已废弃，请使用 [MacroExpansionSource]）
 *
 * 保留此枚举仅为兼容过渡期代码。新代码应使用 [MacroExpansionSource]。
 */
@Deprecated(
    message = "Use MacroExpansionSource instead",
    replaceWith = ReplaceWith("MacroExpansionSource", "org.cangnova.cangjie.macro.service.MacroExpansionSource")
)
