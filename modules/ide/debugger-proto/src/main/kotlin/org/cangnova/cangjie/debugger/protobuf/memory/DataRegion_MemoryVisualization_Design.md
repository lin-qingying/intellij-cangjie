...
class CidrDebuggerHexdumpDataProvider(
    ...
    override fun readOnlyReason(): String? {
        return if (!process.supportsMemoryWrite()) {
            CidrProtoDebuggerBundle.message("proto.debug.memory.view.modify.not.supported", "memory")
        } else {
            when {
                process.session.isStopped ->
                    CidrProtoDebuggerBundle.message("proto.debug.memory.modify.in.stopped.session.is.not.supported")
                !process.session.isSuspended ->
                    CidrProtoDebuggerBundle.message("proto.debug.memory.modify.in.running.session.is.not.supported")
                else -> null
            }
        }
    }
    ...
    private fun createErrorMessage(e: DebuggerCommandException, range: AddressRange): String {
        ...
        return if (reason != null && reason != "Unable to read memory $range") {
            CidrProtoDebuggerBundle.message("proto.debug.error.failedToReadMemory", range, reason)
        } else {
            CidrProtoDebuggerBundle.message("proto.debug.error.failedToReadMemory.unknownReason", range)
        }
    }
}
