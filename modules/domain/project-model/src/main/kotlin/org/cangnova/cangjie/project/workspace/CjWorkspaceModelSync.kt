...
class CjWorkspaceModelSync(private val intellijProject: Project) {
    ...
    companion object {
        ...
        private val CANGJIE_EXTERNAL_SOURCE: ProjectModelExternalSource = object : ProjectModelExternalSource {
            override fun getDisplayName(): String = "CangJie"
            override fun getId(): String = "cn.cangnova.cangjie.workspace"
        }
    }

    private fun createCangJieEntitySource(contentRootUrl: VirtualFileUrl): EntitySource =
    ...
    private fun isCangJieEntitySource(
        ...
        projectRootUrl: String? = null
    ): Boolean {
        if (entitySource !is JpsImportedEntitySource) return false
        if (entitySource.externalSystemId != CANGJIE_EXTERNAL_SOURCE.id) return false
        return projectRootUrl == null || entitySource.virtualFileUrl?.url?.startsWith(projectRootUrl) == true
    }
    ...
    suspend fun syncProject(project: CjProject) {
        ...
        if (project.isWorkspace) {
            ...
            val existingModules = storage.entities(ModuleEntity::class.java)
                .filter { isCangJieEntitySource(it.entitySource) }
            ...
            project.module?.let { module ->
                try {
                    ...
                    val existingModule = existingModules.firstOrNull {
                        isCangJieEntitySource(it.entitySource)
    ...
    private fun detectProjectTypeChange(builder: MutableEntityStorage, project: CjProject): Boolean {
        val allCangjieModules = builder.entities(ModuleEntity::class.java)
            .filter { isCangJieEntitySource(it.entitySource, project.rootDir.url) }
            .toList()
    ...
    private fun cleanupAllProjectModules(builder: MutableEntityStorage, project: CjProject) {
        val projectModules = builder.entities(ModuleEntity::class.java)
            .filter {
                isCangJieEntitySource(it.entitySource, project.rootDir.url)
            }
            .toList()
    ...
    private fun createMainModule(builder: MutableEntityStorage, project: CjProject): ModuleEntity {
        ...
        val entitySource = createCangJieEntitySource(contentRootUrl)
    ...
    private suspend fun buildAllDependencies(
        ...
        val urlManager = WorkspaceModel.getInstance(intellijProject).getVirtualFileUrlManager()
        val entitySource = createCangJieEntitySource(urlManager.getOrCreateFromUrl(cjModule.rootDir.url))
