...
import org.cangnova.cangjie.psi.psiUtil.PsiChildRange
...
import org.cangnova.cangjie.psi.psiUtil.getPrevSiblingIgnoringWhitespace
import org.cangnova.cangjie.psi.psiUtil.getStartOffsetIn
...
import org.cangnova.cangjie.psi.psiUtil.textRange
...
import org.cangnova.cangjie.renderer.DescriptorRenderer.Companion.FQ_NAMES_IN_TYPES
import org.cangnova.cangjie.renderer.DescriptorRendererModifier
import org.cangnova.cangjie.renderer.OverrideRenderingPolicy
...
class CommentSaver(originalElements: PsiChildRange, private val saveLineBreaks: Boolean = false/*TODO?*/) {
    ...
    private val psiFactory = CjPsiFactory(originalElements.first!!.project)
...
//        var debugText: String? = null
    ...
    private class TokenTreeElement(val tokenType: CjToken) : TreeElement()
        ...
        for (commentTreeElement in commentsToRestore) {
            ...
            if (anchor != null) {
                ...
                if (anchor.before) {
                    ...
                    if (restored.tokenType == CjTokens.EOL_COMMENT) {
                    ...
                    if (commentTreeElement.spaceBefore.isNotEmpty()) {
                        when {
                            parent is CjFunctionLiteral ->
                                parent.addBefore(psiFactory.createWhiteSpace(commentTreeElement.spaceBefore), restored)

                            comment.elementType == CjTokens.BLOCK_COMMENT && comment.isMultiLine() ->
    ...
    private fun restoreLineBreaks() {
        for (lineBreakElement in lineBreaksToRestore) {
            ...
            val tokensToMatch = arrayListOf<CjToken>()
    ...
    private fun skipTokensForward(psiElement: PsiElement, tokensToMatch: List<CjToken>): PsiElement? {
    ...
    private fun findFinalAnchorElement(anchor: Anchor, comment: PsiComment): PsiElement {
        ...
        if (anchor.before && comment.tokenType == CjTokens.EOL_COMMENT) {
    ...
    companion object {
        ...
        val PsiElement.tokenType: CjToken?
            get() = node.elementType as? CjToken
    }
}
