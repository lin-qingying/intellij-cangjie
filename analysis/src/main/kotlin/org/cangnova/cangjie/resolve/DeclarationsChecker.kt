...
import org.cangnova.cangjie.diagnostics.DiagnosticFactory0
...
import org.cangnova.cangjie.resolve.binding.BindingContext.Companion.TYPE_PARAMETER
...
//        checkExpectDeclarationModifiers(classOrObject, classDescriptor)
    ...
    private fun checkTypesInClassHeader(classOrObject: CjTypeStatement) {
        fun CjTypeReference.type(): CangJieType? = trace.bindingContext[TYPE, this]
...
//        参数数量
        ...
        if (valueParameters.size > 2) {
            macro.valueParameterList?.let { trace.report(EXCESSIVE_MACRO_PARAMS.on(it)) }
        }
//        检查参数类型
...
//            判断const方法
            ...
            if (!hasBody && inInterface) {
                ...
                if (!hasAbstractModifier && function.hasModifier(CjTokens.OPEN_KEYWORD)) {
...
//        }
        ...
        if (initializer != null) {
            ...
            if (!inInterface && isUninitialized) {
