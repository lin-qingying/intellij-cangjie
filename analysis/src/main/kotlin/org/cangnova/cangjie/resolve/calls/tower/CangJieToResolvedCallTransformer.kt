...
import org.cangnova.cangjie.diagnostics.infos.errors.NEW_INFERENCE_ERROR
import org.cangnova.cangjie.diagnostics.infos.errors.SIGNED_CONSTANT_CONVERTED_TO_UNSIGNED
import org.cangnova.cangjie.diagnostics.infos.warnings.NEW_INFERENCE_DIAGNOSTIC
...
import org.cangnova.cangjie.resolve.binding.BindingContextUtils.updateRecordedType
import org.cangnova.cangjie.resolve.binding.BindingTrace
import org.cangnova.cangjie.resolve.calls.ArgumentTypeResolver
import org.cangnova.cangjie.resolve.calls.DiagnosticReporterByTrackingStrategy
...
import org.cangnova.cangjie.resolve.calls.components.AdditionalDiagnosticReporter
import org.cangnova.cangjie.resolve.calls.components.isArrayType
...
import org.cangnova.cangjie.resolve.calls.util.isFakeElement
...
//            resultingDescriptor.returnType
        ...
        if (parameter != null /*&& isEnabledFor(parameter, context.languageVersionSettings)*/) {
            ...
            context.trace.report(SIGNED_CONSTANT_CONVERTED_TO_UNSIGNED.on(deparenthesized))
        }
    ...
    private fun hasNecessarySafeCall(expression: CjExpression, trace: BindingTrace): Boolean {
        ...
        return expressionType != null && TypeUtils.isOptionType(expressionType)
    }
    ...
    fun reportCallDiagnostic(
        ...
        for (diagnostic in allDiagnostics) {
            ...
            if (shouldReportMissingDiagnostic && REPORT_MISSING_NEW_INFERENCE_DIAGNOSTIC) {
                val factory =
                    if (diagnostic.candidateApplicability.isSuccess) NEW_INFERENCE_DIAGNOSTIC else NEW_INFERENCE_ERROR
                trace.report(
                    factory.on(
