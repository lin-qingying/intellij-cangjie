...
import org.cangnova.cangjie.diagnostics.infos.warnings.DEPRECATED_MODIFIER_FOR_TARGET
import org.cangnova.cangjie.diagnostics.infos.warnings.DEPRECATED_MODIFIER_PAIR
import org.cangnova.cangjie.diagnostics.infos.warnings.REDUNDANT_MODIFIER
import org.cangnova.cangjie.diagnostics.infos.warnings.REDUNDANT_MODIFIER_FOR_TARGET
import org.cangnova.cangjie.lexer.CjKeywordToken
import org.cangnova.cangjie.lexer.CjTokens
import org.cangnova.cangjie.psi.*
import org.cangnova.cangjie.resolve.binding.BindingContext
import org.cangnova.cangjie.resolve.binding.BindingTrace
import org.cangnova.cangjie.resolve.calls.components.getDescriptorKind
            ...
            (descriptor as? DeclarationDescriptorWithSource)?.let { descriptorWithSource ->
                descriptorWithSource.source.getPsi()?.let { psi ->
                    psi.getNameElement()?.let {
                        trace.report(
                            ABSTRACT_MEMBER_VISIBILITY_ERROR.on(
    ...
    private fun checkCompatibility(
        ...
        when (val compatibility = compatibility(firstModifier, secondModifier)) {
            ...
            Compatibility.REPEATED -> if (incorrectNodes.add(secondNode)) {
                trace.report(REPEATED_MODIFIER.on(secondNode.psi, firstModifier))
            }

            Compatibility.REDUNDANT -> trace.report(
                REDUNDANT_MODIFIER.on(
                    secondNode.psi,
                    secondModifier,
                    firstModifier
                )
            ...
            Compatibility.DEPRECATED -> {
                trace.report(DEPRECATED_MODIFIER_PAIR.on(firstNode.psi, firstModifier, secondModifier))
                trace.report(DEPRECATED_MODIFIER_PAIR.on(secondNode.psi, secondModifier, firstModifier))
            }
            ...
            Compatibility.COMPATIBLE_FOR_CLASSES_ONLY, Compatibility.INCOMPATIBLE -> {
                ...
                if (incorrectNodes.add(firstNode)) {
                    trace.report(INCOMPATIBLE_MODIFIERS.on(firstNode.psi, firstModifier, secondModifier))
                }
                if (incorrectNodes.add(secondNode)) {
                    trace.report(INCOMPATIBLE_MODIFIERS.on(secondNode.psi, secondModifier, firstModifier))
                }
            }
        }
    }
...
//            trace.report(
//                 DEPRECATED_MODIFIER_CONTAINING_DECLARATION.on(
//                    node.psi,
//                    modifier,
...
//            trace.report(
//                 WRONG_MODIFIER_CONTAINING_DECLARATION.on(
//                    node.psi,
//                    modifier,
//                    "final expect class"
//                )
    ...
    private fun checkTarget(trace: BindingTrace, node: ASTNode, actualTargets: List<CangJieTarget>): Boolean {
        ...
        when {
            actualTargets.any { it in deprecatedTargets } -> {
                trace.report(
                    DEPRECATED_MODIFIER_FOR_TARGET.on(
                        node.psi, modifier, actualTargets.firstOrNull()?.description ?: "this"
                    )
                )
            }
            ...
            actualTargets.any { it in redundantTargets } -> {
                trace.report(
                    REDUNDANT_MODIFIER_FOR_TARGET.on(
                        node.psi, modifier, actualTargets.firstOrNull()?.description ?: "this"
                    )
                )
            }
    ...
    private fun checkSealed(list: CjModifierList, trace: BindingTrace) {
        if (list.hasModifier(CjTokens.SEALED_KEYWORD)) {
            if (!list.hasModifier(CjTokens.ABSTRACT_KEYWORD) && list.parent !is CjInterface) {
                trace.report(SEALED_ABSTRACT.on(list))
