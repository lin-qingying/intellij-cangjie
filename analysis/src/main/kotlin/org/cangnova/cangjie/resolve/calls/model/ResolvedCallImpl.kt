...
import org.cangnova.cangjie.resolve.calls.tower.CastImplicitClassReceiver
...
class ResolvedCallImpl<D : CallableDescriptor> : MutableResolvedCall<D> {
    ...
    override val knownTypeParametersSubstitutor: TypeSubstitutor?
     ...
     * 设置结果替换器
     */
    fun setResultingSubstitutor(substitutor: TypeSubstitutor) {
     ...
     * 设置已解析调用的替换器
     */
    fun setResolvedCallSubstitutor(substitutor: TypeSubstitutor) {
        // 更新类型参数
        ...
        _typeArguments = _typeArguments.mapValues { (param, type) ->
            substitutor.safeSubstitute(type, param.variance)
        }.toMutableMap()

        // 更新调度接收者类型
        if (_dispatchReceiver is ExpressionReceiver) {
            _dispatchReceiver = (_dispatchReceiver ?: return).replaceType(
                substitutor.safeSubstitute((_dispatchReceiver ?: return).type, Variance.INVARIANT)
            )
        }
    ...
    override fun setSubstitutor(substitutor: TypeSubstitutor) {
    ...
    companion object {
        private val LOG = Logger.getInstance(ResolvedCallImpl::class.java)
        ...
        fun <D : CallableDescriptor> create(
            ...
            return ResolvedCallImpl(candidate, trace, tracing, dataFlowInfoForArguments)
        }
