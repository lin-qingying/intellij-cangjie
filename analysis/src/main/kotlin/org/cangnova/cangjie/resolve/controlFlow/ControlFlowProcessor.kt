...
//                    checkReturnLabelTarget(expression, labeledCjElement)
         ...
         * 2. 如果有初始化器，生成赋值指令
         *
         * 注意：本地变量不需要调用 processLocalDeclaration，
         * 该方法仅用于本地函数（嵌套函数）。
         */
         ...
         * 访问模式匹配变量声明
         *
         * 处理形如 `let (x, y) = tuple` 或 `match expr { case Some(value) => ... }` 的模式匹配变量声明。
         * 模式变量是解构模式的一部分，不是独立的本地声明，因此不调用 processLocalDeclaration。
         * 直接声明变量并处理初始化（如果有）。
         */
         ...
         *
         * 处理类中的字段变量声明（非属性）。
         * 字段变量是类成员，不是本地声明，因此不调用 processLocalDeclaration。
         * 直接声明变量并处理初始化（如果有）。
         */
        override fun visitFieldVariable(field: CjFieldVariable) {
            builder.declareVariable(field)
            val initializer = field.initializer
            if (initializer != null) {
                visitAssignment(field, getDeferredValue(initializer), field)
            }
        }
