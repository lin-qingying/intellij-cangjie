...
import org.cangnova.cangjie.quickDoc.cdoc.CangJieIdeDescriptorRendererHighlightingManager.Companion.eraseTypeParameter
...
import org.cangnova.cangjie.references.CDocReference
...
import org.cangnova.cangjie.highlighter.textAttributesKeyForCjElement
import org.cangnova.cangjie.lexer.cdoc.CDocTemplate
import org.cangnova.cangjie.lexer.cdoc.insert
import org.cangnova.cangjie.lexer.cdoc.psi.impl.*
...
object CDocRenderer {
    ...
    private fun StringBuilder.appendCDocSections(sections: List<CDocSection>) {
        ...
        appendTag(findTagByName("receiver"), CangJieCDocBundle.message("cdoc.section.title.receiver"))

        val paramTags = findTagsByName("param").filter { it.getSubjectName() != null }
        appendTagList(paramTags, CangJieCDocBundle.message("cdoc.section.title.parameters"), CangJieHighlightingColors.PARAMETER)

        val propertyTags = findTagsByName("property").filter { it.getSubjectName() != null }
        appendTagList(
            propertyTags, CangJieCDocBundle.message("cdoc.section.title.properties"), CangJieHighlightingColors.INSTANCE_PROPERTY
        )

        appendTag(findTagByName("constructor"), CangJieCDocBundle.message("cdoc.section.title.constructor"))

        appendTag(findTagByName("return"), CangJieCDocBundle.message("cdoc.section.title.returns"))
        ...
        appendTag(findTagByName("since"), CangJieCDocBundle.message("cdoc.section.title.since"))
        appendTag(findTagByName("suppress"), CangJieCDocBundle.message("cdoc.section.title.suppress"))
    ...
    private fun StringBuilder.appendSamplesList(sampleTags: Sequence<CDocTag>) {
        ...
        appendSection(CangJieCDocBundle.message("cdoc.section.title.samples")) {
            sampleTags.forEach {
                it.getSubjectLink()?.let { subjectLink ->
                    ...
                    this@appendSamplesList.appendStyledCodeBlock(
                        ...
                        if (DumbService.isDumb(subjectLink.project))
                            "// " + CangJieCDocBundle.message("cdoc.comment.unresolved")
                        else when (val target = subjectLink.getTargetElement()) {
                            null -> "// " + CangJieCDocBundle.message("cdoc.comment.unresolved")
    ...
    private fun StringBuilder.appendSeeAlso(seeTags: Sequence<CDocTag>) {
        ...
        appendSection(CangJieCDocBundle.message("cdoc.section.title.see.also")) {
    ...
    private fun StringBuilder.appendAuthors(authorTags: Sequence<CDocTag>) {
        ...
        appendSection(CangJieCDocBundle.message("cdoc.section.title.author")) {
            while (iterator.hasNext()) {
    ...
    private fun StringBuilder.appendThrows(throwsTags: Sequence<CDocTag>, exceptionsTags: Sequence<CDocTag>) {
        ...
        appendSection(CangJieCDocBundle.message("cdoc.section.title.throws")) {
    ...
    fun createHighlightingManager(project: Project): CangJieIdeDescriptorRendererHighlightingManager<CangJieIdeDescriptorRendererHighlightingManager.Companion.Attributes> {
        ...
        return object : CangJieIdeDescriptorRendererHighlightingManager<TextAttributesAdapter> {
            ...
            override val asStatic: TextAttributesAdapter = resolveKey(CangJieHighlightingColors.STATIC_KEYWORD)
