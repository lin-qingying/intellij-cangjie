...
import org.cangnova.cangjie.diagnostics.infos.UNRESOLVED_REFERENCE_DIAGNOSTICS
...
internal class ElementAnnotator(
    ...
    private fun presentationInfo(diagnostics: Collection<Diagnostic>): AnnotationPresentationInfo? {
        ...
        val presentationInfo: AnnotationPresentationInfo = when (factory.severity) {
            Severity.ERROR -> {
                when (factory) {
                    in  UNRESOLVED_REFERENCE_DIAGNOSTICS -> {
                     ...
                     ILLEGAL_ESCAPE -> AnnotationPresentationInfo(
                        ranges, textAttributes = CangJieHighlightingColors.INVALID_STRING_ESCAPE
                    )

                     REDECLARATION -> AnnotationPresentationInfo(
                        ...
                        AnnotationPresentationInfo(
                            ...
                            highlightType =
                                when (factory) {
                                     INVISIBLE_REFERENCE,  DELEGATE_SPECIAL_FUNCTION_MISSING,  DELEGATE_SPECIAL_FUNCTION_NONE_APPLICABLE,  TOO_MANY_ARGUMENTS -> ProblemHighlightType.LIKE_UNKNOWN_SYMBOL
                                    else -> null
                                },
                            textAttributes =
                                when (factory) {
                                     DELEGATE_SPECIAL_FUNCTION_MISSING,  DELEGATE_SPECIAL_FUNCTION_NONE_APPLICABLE,  TOO_MANY_ARGUMENTS -> CodeInsightColors.ERRORS_ATTRIBUTES
                                    else -> null
                                },
                        )
                    }
...
//
            Severity.WARNING -> {
                if (factory ==  UNUSED_PARAMETER && shouldSuppressUnusedParameter(element as CjParameter)) {
                    return null
                }
                ...
                AnnotationPresentationInfo(
                    ...
                    textAttributes = when (factory) {
                         DEPRECATION -> CodeInsightColors.DEPRECATED_ATTRIBUTES
                         UNUSED_ANONYMOUS_PARAMETER -> CodeInsightColors.WEAK_WARNING_ATTRIBUTES
                        else -> null
                    },
