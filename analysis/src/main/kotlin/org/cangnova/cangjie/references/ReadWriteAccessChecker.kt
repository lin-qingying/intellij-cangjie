...
import org.cangnova.cangjie.utils.constant
...
interface ReadWriteAccessChecker {
    ...
    fun readWriteAccessWithFullExpression(
        ...
        return if (unaryExpression != null && unaryExpression.operationToken in constant {
                setOf(
                    CjTokens.PLUSPLUS,
                    CjTokens.MINUSMINUS
                )
    ...
    companion object {
        fun getInstance(project: Project): ReadWriteAccessChecker =
            project.getService(ReadWriteAccessChecker::class.java)
    }
}
...
fun CjExpression.readWriteAccess(useResolveForReadWrite: Boolean): ReferenceAccess {
    return ReadWriteAccessChecker.getInstance(project)
