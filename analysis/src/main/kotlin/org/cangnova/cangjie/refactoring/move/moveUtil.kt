...
import org.cangnova.cangjie.utils.isImported
import java.util.*
...
//        if (descriptor is ClassDescriptor && descriptor.isInner && refExpr.parent is CjCallExpression) return null
        ...
        if (isExtension || containerFqName != null || isImported) return {
            CangJieMoveRenameUsage.Companion.createIfPossible(
                it.mainReference,
                declarationNotNull,
...
internal fun addDelayedImportRequest(elementToImport: PsiElement, file: CjFile) {
    org.cangnova.cangjie.codeinsight.shorten.addDelayedImportRequest(elementToImport, file)
}
