...
object PackageDirectiveCompletion {
    ...
    fun perform(parameters: CompletionParameters, result: CompletionResultSet): Boolean {
        ...
        val variants =
            packageMemberScope.getContributedDescriptors(DescriptorKindFilter.PACKAGES, prefixMatcher.asNameFilter())
        val lookupElementFactory = BasicLookupElementFactory(
            resolutionFacade.project,
            InsertHandlerProvider(
                callType = CallType.PACKAGE_DIRECTIVE,
                editor = parameters.editor,
                expectedInfosCalculator = { emptyList() })
        )
