...
import org.cangnova.cangjie.completion.smart.*
...
class ImportedWeigher(private val classifier: ImportableFqNameClassifier) : LookupElementWeigher("cangjie.imported") {
    private enum class Weight {
        currentPackage,
        preciseImport,
        defaultImport,
...
internal class KotlinLookupElementProximityWeigher : CompletionWeigher() {
    override fun weigh(element: LookupElement, location: CompletionLocation): Comparable<Nothing>? {
        ...
        return PsiProximityComparator.getProximity(
            { psiElement },
            location.completionParameters.position,
            location.processingContext
        )
    }
}

object SmartCompletionPriorityWeigher : LookupElementWeigher("cangjie.smartCompletionPriority") {
...
object KindWeigher : LookupElementWeigher("cangjie.kind") {
    private enum class Weight {
        probableKeyword,
        enumMember,
        callable,
        ...
        return when (val o = element.`object`) {
            ...
            is DescriptorBasedDeclarationLookupObject -> {
                when (val descriptor = o.descriptor) {
                    ...
                    is ClassDescriptor ->  Weight.default
                    else -> Weight.default
                }
            }
...
object CallableWeigher : LookupElementWeigher("cangjie.callableWeight") {
    ...
    private data class CompoundWeight(val weight1: Weight1, val receiverIndex: Int, val weight2: Weight2) :
        Comparable<CompoundWeight> {
...
object VariableOrFunctionWeigher : LookupElementWeigher("cangjie.variableOrFunction") {
    private enum class Weight {
        variable,
        function
    }
 ...
 */
object PreferGetSetMethodsToPropertyWeigher :
    LookupElementWeigher("cangjie.preferGetSetMethodsToProperty", false, true) {
    override fun weigh(element: LookupElement, context: WeighingContext): Int {
        val property =
            (element.`object` as? DescriptorBasedDeclarationLookupObject)?.descriptor as? PropertyDescriptor ?: return 0
...
object DeprecatedWeigher : LookupElementWeigher("cangjie.deprecated") {
...
object PreferMatchingItemWeigher : LookupElementWeigher("cangjie.preferMatching", false, true) {
    private enum class Weight {
        keywordExactMatch,
        defaultExactMatch,
        functionExactMatch,
...
class SmartCompletionInBasicWeigher(
...
) : LookupElementWeigher("cangjie.smartInBasic", true, false) {
    ...
    private val PRIORITY_COUNT = SmartCompletionItemPriority.entries.size
    ...
    private val DESCRIPTOR_TO_SKIP_WEIGHT =
        -1L // if descriptor is skipped from smart completion then it's probably irrelevant
    ...
    override fun weigh(element: LookupElement): Long {
        ...
        val (fuzzyTypes, name) = when (o) {
            ...
            is ThisItemLookupObject -> smartCastCalculator.types(o.receiverParameter)
                .map { it.toFuzzyType(emptyList()) } to null
        ...
        val matched: Collection<Pair<ExpectedInfo, ExpectedInfoMatch>> =
            expectedInfos.map { it to fuzzyTypes.matchExpectedInfo(it) }
...
class PreferContextElementsWeigher(context: DeclarationDescriptor) :
    LookupElementWeigher("cangjie.preferContextElements", true, false) {
...
object ByNameAlphabeticalWeigher : LookupElementWeigher("cangjie.byNameAlphabetical") {
...
object PreferLessParametersWeigher : LookupElementWeigher("cangjie.preferLessParameters") {
    override fun weigh(element: LookupElement): Int? {
        ...
        return if (
            ...
            Registry.`is`("cangjie.auto.completion.prefer.vararg.to.noargs")
        ) {
            -1
        } else {
            size
...
class CallableReferenceWeigher(private val callType: CallType<*>) : LookupElementWeigher("cangjie.callableReference") {
    override fun weigh(element: LookupElement): Int? {
        if (callType is CallType.CallableReference || element.getUserData(SMART_COMPLETION_ITEM_PRIORITY_KEY) == SmartCompletionItemPriority.CALLABLE_REFERENCE) {
            val descriptor =
                (element.`object` as? DescriptorBasedDeclarationLookupObject)?.descriptor as? CallableDescriptor
