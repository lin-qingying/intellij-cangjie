...
import org.cangnova.cangjie.codeinsight.utils.ChooseValueExpression
...
import org.cangnova.cangjie.descriptors.annotations.fqNameOrNull
...
class SpecifyTypeExplicitlyIntention : SelfTargetingRangeIntention<CjCallableDeclaration>(
    ...
    CangJieCodeInsightBundle.lazyMessage("specify.type.explicitly")
), HighPriorityAction {
...
//        if (!ExplicitApiDeclarationChecker.returnTypeCheckIsApplicable(element)) return null
        if (element is CjParameter && element.isLoopParameter ) return null
...
//        ) return null
        ...
        setTextGetter(
            if (element is CjFunction)
                CangJieCodeInsightBundle.lazyMessage("specify.return.type.explicitly")
            else
                defaultTextGetter
        )
        ...
        if (type.isError) {
            if (editor != null && editor !is ImaginaryEditor) {
                HintManager.getInstance()
                    .showErrorHint(editor, CangJieCodeInsightBundle.message("cannot.infer.type.for.this.declaration"))
            }
            return
        }
    ...
    companion object {
        ...
        fun getTypeForDeclaration(declaration: CjCallableDeclaration): CangJieType {
            ...
            val type = (descriptor as? CallableDescriptor)?.let {
                ...
                if (it.overriddenDescriptors.firstOrNull()?.returnType?.isOption == false)
                    it.returnType?.makeNonOption()
                else
                    it.returnType
            }
