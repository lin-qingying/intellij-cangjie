...
import org.cangnova.cangjie.quickfix.SpecifyTypeExplicitlyFix
...
class CangJieFunctionWithLambdaExpressionBodyInspection : AbstractCangJieInspection() {
    ...
    override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean) = object : CjVisitorUnit() {
...
//                if (!used) WrapRunFix() else null
            ...
            holder.registerProblem(
                block,
                CangJieCodeInsightBundle.message("inspection.function.with.lambda.expression.body.display.name"),
    ...
    private class RemoveBracesFix : LocalQuickFix {
        override fun getName() = CangJieCodeInsightBundle.message("remove.braces.fix.text")
    ...
    private class WrapRunFix : LocalQuickFix {
        override fun getName() = CangJieCodeInsightBundle.message("wrap.run.fix.text")
