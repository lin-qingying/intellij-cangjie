...
package org.cangnova.cangjie.types.expressions.unqualifiedSuper

import org.cangnova.cangjie.descriptors.isClass
...
import org.cangnova.cangjie.types.isInterface
...
fun isPossiblyAmbiguousUnqualifiedSuper(
    superExpression: CjSuperExpression,
    supertypes: Collection<CangJieType>
): Boolean =
    supertypes.size > 1 ||
            (supertypes.size == 1 && supertypes.single()
                .isInterface() && isCallingMethodOfAnyWithSuper(superExpression))
...
private fun resolveSupertypesForMethodOfAny(
    ...
    val (typesWithConcreteOverride, isEqualsMigration) = resolveSupertypesByMembers(
        supertypes,
        allowNonConcreteInterfaceMembers = false
    ) {
...
private fun resolveSupertypesByCalleeName(
    supertypes: Collection<CangJieType>,
    calleeName: Name
...
private fun resolveSupertypesByPropertyName(
    supertypes: Collection<CangJieType>,
    propertyName: Name
): Collection<CangJieType> =
    resolveSupertypesByMembers(supertypes, allowNonConcreteInterfaceMembers = true) {
